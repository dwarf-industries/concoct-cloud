...
namespace Platform.DataHandlers
{
    public class FileProcessor
    {
        ...
        internal static (byte[], string) GetFile(string file, string webRootPath)
        {
            using (FileStream fs = File.OpenRead($"{webRootPath}/Files/Uploads/{file}"))
            {
                var fileStreeam = ReadToEnd(fs);
                var fileType = Path.GetExtension(file);
                return (fileStreeam, fileType);
            }
